...
3. \`team_assign_task\` では \`workerId\`、\`objective\`、\`doneCriteria\` だけを渡す。scope、nonGoals、targetPaths、constraintsは追加フィールドにせず \`objective\` へ含め、返されたexecution IDを記録する。queuedは失敗ではない。
...
7. blocked、needs_input、failed、canceledをcompletedへ読み替えない。
...
Worker自体を終了する必要がある場合は \`team_stop_worker\` を \`workerId\` 付きで使い、停止した作業をcompletedとして報告しない。
