...
it('binds tracked, renamed, deleted and untracked binary bytes without staging or committing', async () => {
  ...
  expect((await exec('git', ['-C', f.input.path, 'diff', '--cached', '--name-only'])).stdout).toBe(
    '',
  );
});

it('binds index staging independently from unchanged working bytes', async () => {
  const f = await fixture();
...
it('refuses missing native observation and cross-owner or cross-repository worktrees', async () => {
  const f = await fixture();
  await expect(reviewGraphWorkspace(f.manager, f.input, () => null)).rejects.toThrow('unavailable');
  await expect(
...
it('rejects a symlink type change through the descriptor-relative observation', async () => {
  const f = await fixture();
...
it('rejects bytes changed between inventory and verification observations', async () => {
  const f = await fixture();
