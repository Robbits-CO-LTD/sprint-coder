...
export class XAIProviderClient implements ProviderRuntime {
  ...
  async listModels(
    ...
    const { models, languageModels } = await this.fetchCatalog(
      connection,
      signal,
    );
    ...
    return languageModels.map((languageModel) => {
      ...
      return {
        ...
        reasoning: unknown,
        pricing: {
          promptPerToken: providerValue(
            usdPerToken(languageModel.prompt_text_token_price),
          ),
          completionPerToken: providerValue(
            usdPerToken(languageModel.completion_text_token_price),
          ),
          currency: 'USD',
        },
      };
...
function normalizeHttpError(
  ...
  if (status === 403)
    return error('invalid_request', 'xAI API permission denied', false, status);
  if (status === 404)
    return error('not_found', 'Grok model was not found', false, status);
  if (status === 429)
    return {
