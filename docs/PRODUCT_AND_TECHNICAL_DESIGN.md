...
統合したあと片付けだけに失敗して隔離がquarantinedになった場合（統合したHEADが記録されている）と、Workerの変更をcommitした（WorkerのHEADがある）が統合を記録しないまま隔離がquarantinedになった場合も、完了時には読み直さず統合前の証拠で判定する。Workerの変更をcommitする前に隔離がquarantinedで終わり、実行がfailedまたはcanceledの場合は、その変更はWorkspaceに無いので、そのSagaの検証をLeader Turnの完了条件にしない。統合を記録しないまま隔離が止まったWorkerの変更は、Turnの編集一覧にも出さない（残ったworktreeはTeam画面の一覧で分かる）。記録が読めない・Sagaの束縛と一致しない・Turn自身のWorkspace rootを指す場合や、Graph Missionのように実行がまだ再開できる場合は、Workspaceへの書き込みとして扱い、完了条件を開いたままにする（#568）。
