...
検証時は実際の `node.exe` の絶対パスを使う。Node の major を確認して、その版でテスト分離を切るフラグを選ぶ。Node 22.8 から 23.5 までは `--test --experimental-test-isolation=none`、Node 23.6 以降は `--test --test-isolation=none` を使う（Node 22 系は前者）。どの版でも、`--test` を付けずに `node.exe <テストファイル>` でファイルを1つずつ直接実行すれば、同じプロセスで動く。
