...
| #390 | Merged PR #435; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] 送信中に別 Task へ切り替えると元 Task の入力欄が永久に無効化される |
| #391 | Merged PR #439; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] 背景 Task の Turn が終わってもサイドバーの「実行中」表示が消えない |
| #392 | Merged PR #435; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Skill 一覧の取得に失敗すると設定画面が無限に再取得を繰り返す |
| #393 | Merged PR #439; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Windows・Linux でウィンドウを閉じて終了すると後片付けの大半がスキップされる |
| #394 | Merged PR #435; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Codex で effort に ultra を選ぶと全 Turn が「Turn開始入力を拒否」で失敗する |
| #395 | Merged PR #436; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] CLI が prompt 読み込み前に終了すると Runtime Host プロセスごと落ちる |
| #396 | Merged PR #436; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] CLI の起動失敗時に Turn の後始末が走らず一時ディレクトリや状態が残る |
| #397 | Merged PR #436; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Codex の API 失敗やレート制限が「出力を解釈できませんでした」の再試行不可エラーになる |
| #398 | Merged PR #441; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] 実行完了直前に Team の停止・steer を行うと以後その Task の Team 操作がすべて固まる |
| #399 | Merged PR #441; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] 書き込み可能な Team 実行を steer すると再実行が必ず失敗し Worker が待機のまま残る |
| #400 | Merged PR #436; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] provider レート制限の 3 回目の再試行が配送上限に阻まれ誤ったエラーで失敗する |
| #401 | Merged PR #437; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Managed Local の llama-server がシグナルで落ちると停止操作が 10 秒固まって失敗する |
| #402 | Merged PR #437; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Managed Local の停止に失敗するとライフサイクルが draining のまま固まり以後の Turn が永久に待つ |
...
| #404 | Merged PR #435; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] 自動生成される Task タイトルから識別子中のアンダースコアやバッククォートが消える |
| #405 | Merged PR #438; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] 日本語などのファイル名では未コミットの自分の編集がモデルの変更として差分に混ざる |
| #406 | Merged PR #438; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] エディタ保存が検証済み記述子ではなくパス名でコピーするため差し替え競合で任意ファイルを読める |
| #407 | Merged PR #438; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] コマンド出力が上限に達した後も小さなチャンクが追記され欠落のある出力が連続して見える |
| #408 | Merged PR #439; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Goal 実行中に Stop-and-Send すると無関係な次の Turn で Goal が完了・ブロック扱いになる |
| #409 | Merged PR #436; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] CRLF 形式の SSE がチャンク境界で分断されると応答の一部が黙って消える |
| #410 | Merged PR #440; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Anthropic への要求が 4096 出力トークンに固定され打ち切られても通知されない |
| #411 | Merged PR #436; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] provider の Retry-After を上限なしに待つため Team 実行が数時間止まって見える |
| #412 | Merged PR #436; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] クロスオリジンのリダイレクトで API キーヘッダが転送される |
| #413 | Merged PR #442; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Computer Use で複数文字入力の途中で拒否されると「効果なし」として監査される |
| #414 | Merged PR #442; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Computer Use を緊急停止した直後に「再開可能」な幽霊セッションが残る |
| #415 | Merged PR #439; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] IPC ハンドラの出力側スキーマ違反が「入力内容を確認してください」として表示されログにも残らない |
| #416 | Merged PR #438; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Project コンテキストが Worker 予算超過だと管理ツールセッションが解放されず親 Turn が終わらない |
| #417 | Merged PR #438; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] ツールライフサイクル記録が例外を投げると以後のツール呼び出しが Turn 内で永久に待つ |
...
| #420 | Merged PR #435; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Skill ストアの初回オープンに一度失敗すると再起動まで Skill 機能全体が使えない |
| #421 | Merged PR #437; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] main が未捕捉例外で終了すると llama-server が残留しモデルがメモリに載ったままになる |
| #422 | Merged PR #437; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Managed Local モデルの読み込み中に Turn をキャンセルしても読み込みが止まらない |
| #423 | Merged PR #439; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Project フォルダ運用中に編集が quarantine されても Task に反映されず再起動まで全編集が失敗し続ける |
| #424 | Merged PR #440; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] 高エントロピー判定の誤検知で provider への送信が全面拒否され Turn が原因不明で失敗する |
| #425 | Merged PR #435; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Turn の port 購読を到着前に解除すると main 側の port が開いたまま残る |
| #426 | Merged PR #435; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] ランタイム状態の購読解除関数が捨てられ開発時に二重登録される |
| #427 | Merged PR #438; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] ワークスペース検索ツールだけが root の同一性チェックを通らない |
| #428 | Merged PR #437; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Managed Local モデルのメタデータ読み取りが 8 バイトごとにシステムコールを発行し極端に遅い |
| #429 | Merged PR #436; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Skeptic の完了後も abort リスナーが残り期限切れ時に終了済み Turn へ無駄なキャンセルが飛ぶ |
| #430 | Merged PR #435; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] 失敗診断のコピー中にランタイム状態が変わるとコピーボタンが押せないままになる |
| #431 | Merged PR #442; CLOSED_VERIFIED (user waived ReviewBOT) | [bug] Computer Use の文字入力に制御文字やファンクションキーを含めると許可リスト外のキー操作ができる |
...
- ReviewBOT repeatedly failed externally. The one received concrete finding was fixed in 0e58f68 and its thread resolved. Review completion is not claimed. The user explicitly waived ReviewBOT for this batch and requested close; all 39 repaired Issues were closed as completed and read back as CLOSED, with approval and verification evidence recorded on each Issue.
...
- Remaining OPEN: #333 / #387 / #388 (Computer Use acceptance), #403 / #418 / #419 (triage follow-up), and excluded feature #434. ReviewBOT waiver applies to this repair batch only; it does not mark signed-device or real-Provider acceptance complete.
