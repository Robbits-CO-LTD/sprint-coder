...
## Current checkpoint: product rendering and stored documents
...
- Windows packaged Architecture/Workflow E2E also passed the same operations in CI run 34520023769. Some CI attempts record no iframe click despite complete DOM loading and no script/CSP errors. Visibility alone did not fully resolve this: a later Mac run failed under showInactive. Hosted tests now explicitly focus the window and verify that precondition; the latest CI result is still required. Local E2E retains hidden/non-focus presentation. No iframe/CSP privilege is relaxed.
...
| AC-12      | Typed IR history and separate semantic/render revisions are implemented. Meaningful diff UI, SourceRef snapshots and persisted generation failure state remain.               |
...
Finish the storage checkpoint's current-head validation and hosted pointer-input checks. Then add Main-verified SourceRefs and managed AI proposal/read/update tools through existing Workspace guards, generation failure state and meaningful diff UI. Keep agreement and Mission dispatch out of proposal/render operations. Render revisions/view lease generations remain separate from agreement-bearing semantic revisions. The new v84 must remain distinct from pending #453's v83 through merge.
